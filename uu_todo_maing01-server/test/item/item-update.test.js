...
const {ITEM_CREATE, LIST_CREATE, ITEM_UPDATE, mockDaoFactory} = require("../general-test-hepler");
...
test("HDS", async () => {
  ...
  let response = await TestHelper.executePostCommand(ITEM_UPDATE, updatedData);
...
test("A1 - unsupported keys in dtoIn", async () => {
  ...
  let response = await TestHelper.executePostCommand(ITEM_UPDATE, updatedData);
  expect(response.status).toEqual(200);
  let warning = response.uuAppErrorMap["uu-todo-main/item/updateItem/unsupportedKeys"];
...
test("A2 - dtoIn is not valid", async () => {
  ...
  try {
    ...
    expect(e.code).toEqual("uu-todo-main/item/updateItem/invalidDtoIn");
...
test("A3 - item ist not updated", async () => {
  ...
  try {
    await ItemModel.update("awid", {item: "5cc98c71f400e0581c16ad62", list: "5cc98c71f400e0581c16ad62", text: "name"});
  } catch (e) {
    expect(e.code).toEqual("uu-todo-main/item/updateItem/itemDaoUpdateFailed");
    expect(e.message).toEqual("Item update failed.");
  }
});
